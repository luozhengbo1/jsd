...
<head>
    ...
    <title><?php  echo $coupons['title'];?></title>
...
<div ng-view="" style="height: 100%;" class="ng-scope">
    ...
    <div class="ddb-nav-header ng-scope" common-header="">
        <div class="nav-left-item"
             onclick="location.href='<?php  echo $this->createMobileUrl('detail', array('id' => $coupons['storeid']), true)?>';"><i
    ...
    <div class="main-view ng-scope" id="promotion-show-page">
        <div class="promotion-image" ng-show="promotion.image">
            ...
            <img src="<?php  echo tomedia($coupons['thumb']);?>">
            <div class="promotion-name ng-binding"><?php  echo $coupons['title'];?></div>
        </div>
        <div class="ng-hide">
            <div class="promotion-label">
                <span>活动名称</span>
            </div>
            <div class="promotion-value ng-binding"><?php  echo $coupons['title'];?></div>
        </div>
        <div class="promotion-label"><span>所需积分</span></div>
        <div class="promotion-value ng-binding"><?php  echo $coupons['dcredit'];?>分</div>
        <div class="promotion-label"><span>有效期</span></div>
        <div class="promotion-value ng-binding">
            <?php  echo date('Y-m-d H:i:s', $coupons['starttime']);?> 至 <?php  echo date('Y-m-d H:i:s', $coupons['endtime']);?></div>
        <div class="promotion-label"><span>名额限制</span></div>
        <div class="promotion-value" ng-hide="promotion.usage_limit"><?php  if(empty($coupons['totalcount'])) { ?>不限<?php  } else { ?><?php  echo $coupons['totalcount'];?><?php  } ?></div>
        <div class="promotion-label"><span>适用范围</span></div>
        <div class="promotion-value ng-binding">
            <?php  if($coupons['is_meal']==1) { ?>
            店内
            <?php  } ?>
            <?php  if($coupons['is_delivery']==1) { ?>
            外卖
            <?php  } ?>
            <?php  if($coupons['is_snack']==1) { ?>
            快餐
            <?php  } ?>
            <?php  if($coupons['is_reservation']==1) { ?>
            预定
            <?php  } ?>
        </div>
        <div class="promotion-label"><span>适用商铺</span></div>
        <div class="promotion-value ng-binding"><?php  echo $store['title'];?></div>
        <?php  if(!empty($coupons['content'])) { ?>
        <div class="promotion-label">
            <span>活动介绍</span></div>
        <div class="promotion-value ng-binding" >
            <?php  echo htmlspecialchars_decode($coupons['content'])?>
        </div>
        <?php  } ?>
