...
//                            }
                    ...
                    <div class="item-inner">
                        ...
                        <div class="item-input">
                            <select name="coupon" id="coupon" onchange="select_coupon(this)">
                                ...
                                <option value="<?php  echo $item['ids'];?>"><?php  echo $item['title'];?></option>
                                <?php  } } ?>
                                <?php  } ?>
                            </select>
                        </div>
                ...
                <ul class="full-width-form">
                    <li class="item-content">
                        <div class="item-inner">
                            ...
                            <div class="item-input">
                            	<input maxlength="100" type="text" name="remark" id="remark" placeholder="<?php  echo $store['remarkinfo'];?>">
                            </div>
                        </div>
                    </li>
                    <!--<li class="item-content">-->
                ...
                <ul class="full-width-form">
                    ...
                    <li class="item-content delivery-settlement-content">
                        <div class="item-inner delivery-settlement">
							...
							<!-- <div>
                            配送费-<?php  echo $dispatchprice;?></br/>
							配送方式-<?php  echo $mode;?></br/>
							商家免费金额-<?php  echo $store['freeprice'];?></br/>
							订单总额-<?php  echo $totalprice;?></br/>
							商家配送费-<?php  echo $psf;?></br/>
                            ...
                            <?php  } ?>
                            <?php  if($dispatchprice>0 && $mode==2 && $psnum != 2) { ?>
								<?php  if($store['freeprice']>0) { ?>
									<?php  if($totalprice<=$store['freeprice']) { ?>
										<div>
										配送费用<?php  if($store['freeprice']!=0.00) { ?>(<font color="#f00">满<?php  echo $store['freeprice'];?>免配送费</font>)<?php  } ?>
        ...
        <input type="hidden" id="zhekou" value="<?php  echo $zhekou;?>" name="zhekou">
        <input type="hidden" id="jifen_dk" value="<?php  echo $jifen_dk;?>" name="jifen_dk">
        <input type="hidden" id="xin_jiang" value="<?php  echo $coupon_obj1['dmoney'];?>" name="xin_jiang">
        <input type="hidden" id="man_jiang" value="<?php  echo $coupon_obj2['dmoney'];?>" name="man_jiang">
        ...
        <input type="hidden" name="psnum" id="psnum" value="<?php  echo $psnum;?>">
        <input type="hidden" name="distance" id="distance" value="<?php  echo $distance;?>">
        <input type="hidden" name="delivery_radius" id="delivery_radius" value="<?php  echo $delivery_radius;?>">
    ...
    <nav class="bar bar-tab">
        ...
        </span>         <a href="#" class="button button-fill delivery-button-submit" id="btnselect">
            <?php  if($append ==2) { ?>
            确认加菜
            <?php  } else { ?>
            确认下单
...
//            return false;
        ...
        if (true) {
           ...
           address = 'aa';
            ...
            var lat = $("#lat").val();
            var lng = $("#lng").val();
            var zhekou = $("#zhekou").val();//会员折扣
            var psnum = $("#psnum").val();//配送方式
            console.log(psnum);
            $.ajax({
    ...
    function tototal(){
        //var goodstotal = parseFloat($("#goodsprice").val());totalprice
        ...
        var zhekou = $("#zhekou").val();//会员折扣
        var jifen_dk = $("#jifen_dk").val();//积分抵扣

        var xin_jiang = $("#xin_jiang").val();//新用户优惠
        var man_jiang = $("#man_jiang").val();//满减优惠
        //var dlimitprice = $("#dlimitprice").val();
        console.log('xin_jiang======'+xin_jiang)
        console.log('man_jiang======'+man_jiang)
        ...
        var pricedetail = '';
        if (mode == 2) { // 外卖
            endTotal = goodstotal + dispatchprice - zhekou;//商品 + 配送费
            ...
            endTotal = (goodstotal + totalteavalue - zhekou);//商品 + 茶位费
        } else {
            endTotal = goodstotal;
        }
        ...
        //积分抵扣
        if(jifen_dk >0){
            endTotal = endTotal - jifen_dk;
        }
        //新用户优惠
        if(xin_jiang !=''){
            endTotal = endTotal - xin_jiang;
        }
        //满减
        if(man_jiang !=''){
            endTotal = endTotal - man_jiang;
        }
        ...
        // var goodstotalbf = endTotal;
        // console.log("备份"+goodstotalbf);
...
//        changeBtnSelect();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(totalM - gmoney >= 0){
                        ...
                        $('#dprice').val('0');
                        $("#tishi").text("优惠券不可用").css("display","block");
                        setTimeout(function () {
                            $("#tishi").css("display","none");
                        },1500);
                    }
                    
                    //$('#gmoney').val(data.gmoney);//条件金额
                    // if (data.status == 1) {
    ...
    function checkItem() {
        ...
        if(Number(distance) > Number(delivery_radius)){
            $.mvalidateTip('地址超出配送范围，请重新选择地址');
            return false;
        }
        totalprice = parseFloat($("#totalprice").val());
//        if (totalprice <= 0) { ("金额为0，请选择商品！"); return false; }
