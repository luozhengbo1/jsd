...
class Core extends WeModuleSite
{
    ...
    public $table_wmtype = 'weisrc_dish_wmtype';
    public $table_coupon = 'weisrc_dish_coupons';
    public $table_coupons = 'weisrc_dish_coupons';
    ...
    public $table_member = "weisrc_dish_member";
    public $table_members = "weisrc_dish_members";
    ...
    public $table_grant = "weisrc_dish_grant";
        ...
        if ($_W['role'] == 'operator') {
            ...
            $navemenu[0] = array(
                ...
                'items' => array(
                    ...
                    14 => $this->createMainMenu('平台管理 ', $do, 'integral', ''),
                    15=> $this->createMainMenu('平台积分 ', $do, 'money', ''),
                    16=> $this->createMainMenu('平台运营费添加 ', $do, 'cost', ''),
                )
            );
...
//        $cur_color = ' style="color:#d9534f;" ';
        ...
        $navemenu[] = array(
            ...
            'items' => array(
                ...
                5 => $this->createSubMenu('达达配送 ', $do, 'dada', 'fa-angle-right', $cur_color, $storeid),
            )
        );
        $cur_color = '#8d8d8d';
        $navemenu[] = array(
	...
	*计算两个经纬度之间驾车行驶距离
	*/
	function getDistanceByGaodeForDrive($lat1, $lng1, $lat2, $lng2, $len_type = 1, $decimal = 2)
    {
		$url="https://restapi.amap.com/v3/direction/driving?origin=".$lng1.",".$lat1."&destination=".$lng2.",".$lat2."&extensions=base&strategy=16&output=json&key=8bd907c04a277af453449a32c33d57ab";
		$content = file_get_contents($url);
		$arr = json_decode($content,true);
		$s = 0;
		if(!empty($arr) && $arr['status'] == 1 && strtolower($arr['info']) == "ok")
		{
			if($arr['count'] > 1)
			{
				$distance = array();
				$index = 0;
				$mindistance = 0;
				foreach($arr as $list=>$route){
					if(is_array($route)){
						foreach($route as $rlist=>$paths){
							if(is_array($paths)){
								foreach($paths as $plist=>$v){
				...
				$s = round(($mindistance / 1000), $decimal);
			}
			else{
				$s = round(($arr['route']['paths']['0']['distance'] / 1000), $decimal);
			}
		}
		return $s;
    }
	...
	*/
	function getDistanceByGaodeForRiding($lat1, $lng1, $lat2, $lng2, $len_type = 1, $decimal = 2)
    {
		...
		$arr = json_decode($content,true);
		$s = 0;
		if(!empty($arr) && $arr['errcode'] == 0 && strtolower($arr['errmsg']) == "ok")
		{
			$s = round(($arr['data']['paths']['0']['distance'] / 1000), $decimal);
		}
		return $s;
    }
