...
if ($operation == 'display') {
    ...
    $condition .= " AND a.storeid = 0 AND a.deleted = 0";
    $pindex = max(1, intval($_GPC['page']));
    $psize = 8;

    $goods_name_sql = "select distinct a.goodsid,b.title from
            ".tablename($this->table_goods_activity)."as a left join
            " .tablename('weisrc_dish_goods')." as  b on  b.id=a.goodsid  where  a.weid=:weid and a.storeid=:storeid ORDER BY a.id DESC,a.id DESC ";
    $goods_name = pdo_fetchall($goods_name_sql,  array(':weid' => $weid, ':storeid' => 0));
    ...
    if (checksubmit('submit')) {
        ...
        $sql_activity = "select * from " . tablename($this->table_goods_activity) . "as c where c.deleted = 0 and c.goodsid = {$data['goodsid']} and c.weid=:weid and  not (c.`startdate`>{$data["enddate"]} or c.`enddate`<{$data["startdate"]})";
