...
//        $this->serverip = getServerIP();
    ...
    public function doEmail($title){
        global $_W, $_GPC;
        $weid = $this->_weid;
        $resarr = pdo_fetch("SELECT * FROM ".tablename('weisrc_dish_cost_email')." WHERE weid = :weid",array(":weid"=>$weid));
        //var_dump($resarr);exit();
        $smtpemailto = $resarr['mailhostname'];//'';//发送给谁
        $mailtitle = "平台运营费通知";//邮件主题
        $mailcontent = "通知详情：".$title."";
        ...
        foreach ($arr_mailto as $v_mailto) {
            $mail->addAddress($v_mailto,'平台运营费通知');
        }
    ...
    public function doDada($weid,$orderid,$storeid){
        ...
        $store=pdo_get('weisrc_dish_stores',array('weid'=>$weid,'id'=>$storeid));
...
ismerge=0 AND status=3 AND (paytype=1 OR paytype=2 OR paytype=4) ";
        ...
        if ($is_contain_delivery == 1) { //包含配送

            $order_totalprice = pdo_fetchcolumn("SELECT sum(totalprice-one_order_getprice-refund_price+pt_yf+dprice+zhekou+jifen_dk) FROM " . tablename($this->table_order) . " WHERE {$strwhere}", array(':weid' => $weid, ':storeid' => $storeid));
            $order_totalprice = floatval($order_totalprice);
            //var_dump($order_totalprice);exit();
        } else {
            //不包含配送费
            $time = strtotime('2017-07-17 00:00:00');
            $order_totalprice1 = pdo_fetchcolumn("SELECT sum(totalprice-one_order_getprice-refund_price+pt_yf+dprice+zhekou+jifen_dk) FROM " . tablename($this->table_order) . " WHERE {$strwhere} AND
dateline<:dateline", array(':weid' => $weid,
                ':storeid' => $storeid, ':dateline' => $time));
            $order_totalprice1 = floatval($order_totalprice1);

            $order_totalprice2 = pdo_fetchcolumn("SELECT sum(totalprice-dispatchprice-one_order_getprice-refund_price+pt_yf+dprice+zhekou+jifen_dk) FROM " . tablename($this->table_order) . " WHERE {$strwhere}
AND dateline>=:dateline", array(':weid' => $weid,
    ...
    public function getStoreOutTotalPrice($storeid)
    {
		...
		//sjg 2018-09-04
		//$totalprice1 = intval($totalprice1 * pow(10, 2))/ pow(10, 2);
        return $totalprice1;
    }
    ...
    public function getStoreGetTotalPrice($storeid)
    {
		...
		//sjg 2018-09-04
		//$totalprice2 = intval($totalprice2 * pow(10, 2))/ pow(10, 2);
        return $totalprice2;
    }
...
from_user=:from_user AND optionid=:optionid ", array(':goodsid' => $dishid, ':weid' => $weid, ':storeid' => $storeid, ':from_user' => $from_user, ':optionid' => $optionid));
    ...
    public function getTablesQrcode($tablesid)
    {
        global $_W, $_GPC;
...
//  OR agentid2=:agentid))  ", array(':weid' => $_W['uniacid'], ':from_user' => $value['from_user'], ':agentid' => $value['id']));
    ...
    public function out_order($commoncondition, $paras)
    {
        ...
        foreach ($list as $key => $value) {
            ...
            $arr[$i]['pt_yf'] = $value['pt_yf'];
            $arr[$i]['sj_yf'] = $value['sj_yf'];
            $arr[$i]['dprice'] = $value['dprice'];
            $arr[$i]['zhekou'] = $value['zhekou'];
            $arr[$i]['jifen_dk'] = $value['jifen_dk'];
            $arr[$i]['pt_bu'] = $value['pt_yf']+$value['dprice']+$value['zhekou']+$value['jifen_dk'];
            $arr[$i]['ps_jvli'] = $value['ps_jvli'];
 ...
 DESC LIMIT 1", array(':weid' => $this->weid, ':couponid' => $value['couponid']));
            ...
            $arr[$i]['coupon_info'] = $value['newlimitpricevalue']+$value['oldlimitpricevalue'];
            if (!empty($value['newlimitprice'])) {
                $arr[$i]['newlimitprice'] = $value['newlimitpricevalue'];
            } else {
                $arr[$i]['newlimitprice'] = '';
            }
            if (!empty($value['oldlimitprice'])) {
                $arr[$i]['oldlimitprice'] = $value['oldlimitpricevalue'];
            } else {
                $arr[$i]['oldlimitprice'] = '';
            }
...
BY id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':couponid' => $id));
    ...
    public function doWebQueryFans()
    {
        ...
        //var_dump($kwd);
        //查询所有会员
        // $mem = pdo_fetchall("SELECT * FROM".tablename($this->table_fans)."where weid = :weid",array(':weid'=>$_W['uniacid']));
        // echo "<pre>";
        // print_r($mem);
        // echo "</pre>";exit();
...
givetime<:givetime", array(':weid' => $weid, ':from_user' => $from_user, ':givetime' => TIMESTAMP));
    ...
    function getQrcodeData($oid, $size = 0)
    {
//        global $_W;
...
//                $content .= '备注:' . $order['remark'] . $huanhang;
            ...
            if ($order['status'] == -1) {
                ...
                if(!empty($jifen)){
                    ...
                    pdo_update("mc_members",array('credit1'=>$credit1+$jifen['limit_discount']),array('uid'=>$user['uid']));//下单增加积分
                }
                return;
            }
...
//                                $this->sendAdminOrderEmail($value['email'], $order, $store, $goods_str);
                    ...
                    if ($order['dining_mode'] == 2 && $setting['delivery_mode'] != 1) { //外卖订单,通知配送
                        ...
                        if ($setting['delivery_mode'] == 2) { //所有配送员
                            $deliverys = pdo_fetchall("SELECT * FROM " . tablename($this->table_account) . " WHERE weid = :weid AND role=4 AND status=2 AND (storeid='' OR
storeid=".$order['storeid'].") ORDER BY id DESC ", array(':weid' => $this->_weid));
                            ...
                            if ($areaid != 0) {
                                $strwhere = " AND areaid={$areaid} AND (storeid='' OR
storeid=".$order['storeid'].") ";
...
//                                $this->sendAdminOrderEmail($value['email'], $order, $store, $goods_str);
        ...
        if (empty($order)) {
            //message('订单不存在123!!!');
            $endtime = time()+30*24*3600;//到期日期
            $huiyuan = array('weid'=>$weid,'user'=>$params['user'],'fee'=>$params['fee'],'title'=>'会员优惠','time'=>time(),'endtime'=>$endtime);
            $beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
            $endToday=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
            $hui = pdo_fetch("SELECT * FROM".tablename('weisrc_dish_member_ka')."where weid = '{$weid}' and user='{$params['user']}' and time >{$beginToday} and time<{$endToday} ");
            if(empty($hui)){
                pdo_insert("weisrc_dish_member_ka",$huiyuan);
            }
            $url = '../../app/' . $this->createMobileUrl('usercenter');
            //message('支付成功123!!!');
        }else{
        ...
        if ($params['type'] == $credit) {
            ...
            if ($params['type'] == 'baifubao') {
                ...
                message($tip_msgs, $this->createMobileUrl('orderdetail', array('orderid' => $orderid), true), 'success');
            }
        } else {
