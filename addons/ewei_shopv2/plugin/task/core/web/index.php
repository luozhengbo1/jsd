...
//          二级奖励
            ...
            //保存回复规则
            $this->replyRule($data['keyword_pick'], $saveRet);
     ...
     * @param $keyword
     */
    public function replyRule($keyword, $taskid){
        global $_W, $_GPC;
        // 任务id
        ...
        // 看下关键字是否已经存在
        $keywordExists = pdo_fetchcolumn("select count(1) from ". tablename('rule_keyword'). " where `uniacid` = {$_W['uniacid']} and `module` = 'ewei_shopv2' and `rid` = {$rid} and `content` = '{$keyword}' and `taskid` <> {$taskid}");

        if ($keywordExists) {
            show_json(0, "关键字已存在，请修改重试！");
        }

        $id = pdo_fetchcolumn("select id from ".tablename('rule_keyword')." where uniacid = {$_W['uniacid']} and module = 'ewei_shopv2' and rid = {$rid} and taskid = {$taskid} and taskid <> 0 ");
        ...
        if (empty($id)){
            ...
            pdo_update('rule_keyword', array('content'=>$keyword), array('id'=>$id));
        }
    }
