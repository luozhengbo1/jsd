...
class Extension_EweiShopV2Page extends PluginWebPage {
    ...
    function __construct(){
        ...
        switch ($this->action){
            case 'single':
                $this->taskType = '单次任务';break;
            case 'repeat':
                $this->taskType = '重复任务';break;
            case 'first':
                $this->taskType = '新手任务';break;
            case 'period':
                $this->taskType = '周期任务';break;
            case 'point':
                $this->taskType = '目标任务';break;
            default:
                exit('不存在的任务类型');
        }
    ...
    function add(){
        ...
        if ($_W['ispost']){
            ...
            switch ($this->action){
                case 'single': $data['type'] =1;break;
                case 'repeat': $data['type'] =2;break;
                case 'first': $data['type'] =3;break;
                case 'period': $data['type'] =4;break;
                case 'point': $data['type'] =5;break;
            ...
            if (empty($id)){
                ...
                $taskid = pdo_insertid();
                if (!empty($taskid)) show_json(1, array('url'=>webUrl('task.extension.'.$this->action,array('taskfunc'=>'add','id'=>$taskid))));
            }else{
                if (pdo_update("ewei_shop_task",$data,array('id'=>$id, 'uniacid'=>$_W['uniacid']))){
    ...
    function goods(){//商品选择器
        ...
        if (!empty($_GPC['title'])) $title = trim($_GPC['title']);
        if(!empty($type)){
            if($type=='goods'){
