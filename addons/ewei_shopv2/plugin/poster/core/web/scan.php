...
class Scan_EweiShopV2Page extends PluginWebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT m.avatar,m.nickname,m.realname,m.mobile,m1.avatar as avatar1,m1.nickname as nickname1,m1.realname as realname1,m1.mobile as mobile1,scan.scantime FROM " . tablename('ewei_shop_poster_scan') . " scan "
				. " left join " . tablename('ewei_shop_member') . ' m1 on m1.openid = scan.openid '
				. " left join " . tablename('ewei_shop_member') . ' m on m.openid = scan.from_openid '
				. " WHERE 1 {$condition}  ORDER BY scan.scantime desc "
				. "  LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('ewei_shop_poster_scan') . " scan "
				. " left join " . tablename('ewei_shop_member') . ' m1 on m1.openid = scan.openid '
				. " left join " . tablename('ewei_shop_member') . ' m on m.openid = scan.from_openid '
