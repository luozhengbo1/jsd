...
class Index_EweiShopV2Page extends PluginMobilePage {
	...
	function main() {
        ...
        if($item['type']==0){
            ...
            if($item['qrcode']==0){
            ...
            }elseif($item['qrcode']==1){
                $qrcode = $this->model->getQR(array(
                    'openid'=>$openid,
                    'invitationid'=>$id,
                    'roomid'=>$roomid,
                    'mid'=>$mid
                ...
                /*
                $qrcode = $qrcode['qrimg'];
                $qrcode = mobileUrl('invitation/getqr', array('qrcode'=>$qrcode), true);
                $qrcode = 'https://dake.foxdev.cn/addons/ewei_shopv2/plugin/invitation/static/images/baidu.png';*/
            }
        }else{
        ...
        $json = json_encode(array(
            'bgImg'=>'',
            'logoimg'=>tomedia($_W['shopset']['shop']['logo']),
            'headImg'=> !empty($member['avatar']) ? $member['avatar'] : tomedia($_W['shopset']['shop']['logo']),
            'qrCode'=>$qrcode,
        ...
        $_W['shopshare'] = array(
            'title' => $member['nickname']. '制作了自己的邀请卡',
            'imgUrl' => tomedia($templist[0]['nail']),
            'desc' => '你也来试试，超好玩的！',
            'link' => mobileUrl('invitation', array('id'=>$id,'mid'=>$mid), true) ,
        );
        if($item['type']==0){
	...
	protected function getTemp($item) {
        ...
        foreach ($item['data']['selected'] as $tempid){
            if(intval($tempid)>0){
                // 默认模板
                $templist[] = array(
                    'bg'=>'../addons/ewei_shopv2/plugin/invitation/static/templist/image_'. $tempid. '_bg.jpg',
                    'nail'=>'../addons/ewei_shopv2/plugin/invitation/static/templist/image_'. $tempid. '_nail.jpg'
                );
            }else{
                // 自定义模板
                ...
                $templist[] = array(
                    'bg'=>tomedia($item['data']['upload'][$tempid]['bg']),
                    'nail'=>tomedia($item['data']['upload'][$tempid]['nail'])
                );
            }
        }
        unset($item);
