...
class InvitationProcessor extends PluginProcessor {
	...
	private function handle($obj, $follow=false) {
		...
		$invitation = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_invitation'). ' WHERE id=:id AND status=1 AND qrcode=1 AND uniacid=:uniacid', array(
		    ':id'=>$qr['invitationid'],
            ':uniacid'=>$_W['uniacid']
        ));
        ...
        if($invitation['type']==0 && p('live')){
            // 1. 查询 直播间是否存在
            $room = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_live'). ' WHERE id=:id AND status=1 AND uniacid=:uniacid', array(
                ':id'=>$qr['roomid'],
                ':uniacid'=>$_W['uniacid']
            ));
            if(empty($room)){
            ...
            $news = array(array(
                'title'=>!empty($room['share_title'])? $room['share_title']: $room['title'],
                'picurl'=>!empty($room['share_icon'])? tomedia($room['share_icon']): tomedia($room['thumb']),
                'description'=>!empty($room['share_desc'])? $room['share_desc']: $this->filter($room['introduce']),
                'url'=> $_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&m=ewei_shopv2&do=mobile&r=live.room&id='. $room['id']. '&mid='.$member['id']
            ));
        }else{
            $this->responseDefault($obj, 4);
        }
