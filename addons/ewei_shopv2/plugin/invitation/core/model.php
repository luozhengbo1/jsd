...
class InvitationModel extends PluginModel {
    ...
    public function getQR($params=array()) {
        ...
        // 1. 查询用户二维码
        $qr = pdo_fetch('select * from ' . tablename('ewei_shop_invitation_qr') . ' where openid=:openid and acid=:acid and invitationid=:invitationid and roomid=:roomid limit 1', array(
        ...
        if(empty($qr)){
            ...
            $ims_qrcode = array(
                ...
                'qrcid' => $scene_id,
                'model' => 0,
                'name' => 'EWEI_SHOP_INVITATION_QRCODE',
                'keyword' => 'EWEI_SHOP_INVITATION',
                'expire' => $expire,
                'createtime' => time(),
                'status' => 1,
                'url' => $result['url'],
    ...
    protected function getSceneID() {
        ...
        while (1) {
            ...
            $count = pdo_fetchcolumn('select count(*) from ' . tablename('qrcode') . ' where qrcid=:qrcid and acid=:acid and model=0 limit 1', array(
                ':qrcid' => $scene_id,
                ':acid' => $_W['acid']
            ));
            if ($count <= 0) {
    ...
    public function getQRByTicket($ticket) {
        ...
        $qr = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_invitation_qr'). ' WHERE ticket=:ticket AND acid=:acid', array(
            ':ticket'=>$ticket,
            ':acid'=>$_W['acid']
        ));
        if(empty($qr)){
    ...
    public function scanLog($openid=null, $qr=array(), $follow=false) {
        ...
        // 更新扫描记录
        pdo_query('UPDATE '. tablename('ewei_shop_invitation').' SET scan=scan+1 WHERE id=:id AND uniacid=:uniacid', array(
            ':id'=>$qr['invitationid'],
            ':uniacid'=>$_W['uniacid'],
        ));
        ...
        // 更新关注数量
        if($follow){
            pdo_query('UPDATE '. tablename('ewei_shop_invitation').' SET follow=follow+1 WHERE id=:id AND uniacid=:uniacid', array(
                ':id'=>$qr['invitationid'],
                ':uniacid'=>$_W['uniacid'],
            ));
        }
        // 插入扫描记录
        pdo_insert('ewei_shop_invitation_log', array(
            'uniacid'=>$_W['uniacid'],
            'invitation_id'=>$qr['invitationid'],
            'openid'=>$openid,
            'invitation_openid'=>$qr['openid'],
            'scan_time'=>time(),
            'follow'=>$follow? 1: 0
        ));
    }
