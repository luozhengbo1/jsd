define(["biz"], function (biz) {
        ...
        $('#btn-check').click(function () {
            ...
            model.check = true;
            model.myArray.splice(0,model.myArray.length);//清空数组
            $('input[name=ischeck]').attr('value',1);
            ...
            $.ajax({
                ...
                success: function (htm) {
                    ...
                    $.each(model.myArray,function (index,item) {
                        ...
                        if(model.check){
                        $('input[data-name='+item+'][value=-1]').prop("checked", "checked");//记录审核不通过的
                        }else{
                            $('input[data-name='+item+'][value=2]').prop("checked", "checked"); //记录审核通过的
                        }
                    ...
                    for(var key in model.content){
                        ...
                        $('input[data-name='+key+']').val(model.content[key]); //记录审核通过的
                    }
                    }
                    ...
                    $('input[type=radio]').click(function () {
                        ...
                        if(model.check){
                            if(input_val==-1){
                                ...
                                model.myArray.splice($.inArray(input_name,model.myArray),1);//移除数组元素
                            }
