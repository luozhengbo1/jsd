...
class User_EweiShopV2Page extends PluginWebPage {
    ...
    function main() {
        ...
        $psize = 100;
            ...
            $sql = "select dm.id,dm.diycommissionfields,dm.diycommissionfields,dm.diymemberfields,dm.diymemberdata,dm.nickname,dm.realname,dm.avatar,l.levelname from " . tablename('ewei_shop_member') . " dm "
                . " left join " . tablename('ewei_shop_member') . " lm on lm.agentid = dm.id"
                ...
                . " where dm.uniacid = " . $_W['uniacid'] . " and dm.isagent =1 and lm.isagent =1 and dm.status=1 and lm.status=1  group by dm.id ORDER BY dm.agenttime desc";

            $sql.=" LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
            $userlist = pdo_fetchall($sql, $params);
        if (!empty($searchstart)) {
            $userlist = pdo_fetchall($sql, $params);
            $total_sql = "select COUNT(1) from " . tablename('ewei_shop_member') . " dm "
                . " left join " . tablename('ewei_shop_member') . " lm on lm.agentid = dm.id"
                . " left join " . tablename('ewei_shop_commission_level') . " l on l.id = dm.agentlevel"
                . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid and f.uniacid={$_W['uniacid']}"
                . " where dm.uniacid = " . $_W['uniacid'] . " and dm.isagent =1 and lm.isagent =1 and dm.status=1 and lm.status=1";
