...
class Down_EweiShopV2Page extends CommissionMobileLoginPage {
	...
	function main() {
		...
		if($this->set['level']>=2 && $levelcount1>0){
		...
		if($this->set['level']>=3 && $levelcount2>0){
	...
	function get_list() {
		...
		if ($level == 1) {
				...
				$hasangent = true;
				$total_level =  pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_member').' where agentid=:agentid and uniacid=:uniacid limit 1'
			,array(":agentid"=>$member['id'],':uniacid'=>$_W['uniacid']));
		...
		foreach ($list as &$row) {
			...
			$moneycount = pdo_fetchcolumn('select sum(og.realprice) from ' . tablename('ewei_shop_order_goods') . ' og '
					. ' left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id where o.openid=:openid  and o.status>=1 and o.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
