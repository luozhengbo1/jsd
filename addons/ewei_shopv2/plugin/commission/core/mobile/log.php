...
class Log_EweiShopV2Page extends CommissionMobileLoginPage {
	...
	function get_list() {
		...
		$commissioncount = pdo_fetchcolumn('select sum(commission) from '.tablename('ewei_shop_commission_apply') ." where mid=:mid and uniacid=:uniacid and status>-1 limit 1",
	...
	function orders() {
		...
		$list = pdo_fetchall("select o.id,o.agentid, o.ordersn,o.price,o.goodsprice, o.dispatchprice,o.createtime, o.paytype from " . tablename('ewei_shop_order') . ' o '
				...
				. " where  o.id in ( " . implode(",", $ids) . " ) LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
