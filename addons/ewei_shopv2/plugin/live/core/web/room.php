    ...
    protected function post() {
        ...
        if ($_W['ispost']){
            $data = array(
                ...
                'liveidentity' => trim($_GPC['liveidentity']),
                'url' => trim($_GPC['url']),
                'video' => trim($_GPC['video']),
                'thumb' => save_media($_GPC['thumb']),
                'cover' => save_media($_GPC['cover']),
                'covertype' => intval($_GPC['covertype']),
                'goodsid' => $goodsids,
                'iscoupon' => intval($_GPC['iscoupon']),
                'couponid' => $_GPC['couponid'],
                'livetype' => intval($_GPC['livetype']),
                'screen' => intval($_GPC['screen']),
                'category' => intval($_GPC['category']),
                'livetime' => strtotime($_GPC['livetime']),
                'recommend' => intval($_GPC['recommend']),
                'hot' => intval($_GPC['hot']),
                'status' => intval($_GPC['status']),
                'introduce' => m('common')->html_images($_GPC['introduce']),
                'packetmoney' => floatval($_GPC['packetmoney']),
                'packettotal' => intval($_GPC['packettotal']),
                'packetprice' => floatval($_GPC['packetprice']),
                'packetdes' => trim($_GPC['packetdes']),
                ...
                'showcommission' => is_array($_GPC['showcommission']) ? implode(",", $_GPC['showcommission']) : '',
                'jurisdiction_url' => trim($_GPC['jurisdiction_url']),
                'jurisdictionurl_show' => intval($_GPC['jurisdictionurl_show']),
                'notice' => trim($_GPC['notice']),
                'notice_url' => trim($_GPC['notice_url']),
            ...
            //show_json(0,$nestable);
            $tabs = array(
                'interaction'=>array(
                    'name'=>trim($_GPC['dtab']['tabtitle']),
                    'isshow'=>1,
                ),
                'goods'=>array(
                    'name'=>trim($_GPC['dtab']['goodstitle']),
                    'isshow'=>$_GPC['dtab']['goodstitleshow']==='0'?0:1,
                ),
                'introduce'=>array(
                    'name'=>trim($_GPC['dtab']['introducetitle']),
                    'isshow'=>$_GPC['dtab']['introducetitleshow']==='0'?0:1,
                ),
                'customname1'=>array(
                    'name'=>trim($_GPC['dtab']['customname1']),
                    'type'=>trim($_GPC['dtab']['customtype1']),
                    'url'=>$_GPC['dtab']['customurl1'],
                    'introduce'=>m('common')->html_images($_GPC['dtab']['customintroduce1']),
                    'isshow'=>in_array('customname1',$nestable)?1:0,
                ),
                'customname2'=>array(
                    'name'=>trim($_GPC['dtab']['customname2']),
                    'type'=>trim($_GPC['dtab']['customtype2']),
                    'url'=>$_GPC['dtab']['customurl2'],
                    'introduce'=>m('common')->html_images($_GPC['dtab']['customintroduce2']),
                    'isshow'=>in_array('customname2',$nestable)?1:0,
                ),
                'customname3'=>array(
                    'name'=>trim($_GPC['dtab']['customname3']),
                    'type'=>trim($_GPC['dtab']['customtype3']),
                    'url'=>$_GPC['dtab']['customurl3'],
                    'introduce'=>m('common')->html_images($_GPC['dtab']['customintroduce3']),
                    'isshow'=>in_array('customname3',$nestable)?1:0,
                ),
                'customname4'=>array(
                    'name'=>trim($_GPC['dtab']['customname4']),
                    'type'=>trim($_GPC['dtab']['customtype4']),
                    'url'=>$_GPC['dtab']['customurl4'],
                    'introduce'=>m('common')->html_images($_GPC['dtab']['customintroduce4']),
                    'isshow'=>in_array('customname4',$nestable)?1:0,
                ),
            );
                ...
                foreach ($_GPC['couponid'] as $key => $value){
                    if(strpos($couponid, $value)!==false){
                        ...
                        $data_coupon = array(
                            'uniacid'=>$uniacid,
                            'roomid'=>$id,
                            'couponid'=>intval($value),
                            'coupontotal'=>intval($_GPC['coupontotal'.$value.'']),
                            'couponlimit'=>intval($_GPC['couponlimit'.$value.''])
                        );
            ...
            if (!empty($id)) {
                /*更新直播时间，订阅通知清零*/
                if($data['livetime']>$item['livetime']){
            ...
            if(!empty($goodsarr)){
                foreach($goodsarr as $key => $value){
                    ...
                    //show_json(0,$live_goods);
                    if(empty($live_goods)){
                        $live_goods_data = array(
                            'uniacid' => $uniacid,
                            'goodsid' => $value['id'],
                            'liveid' => $id,
                            'minliveprice' => floatval($minliveprice),
                            'liveprice' => floatval($liveprice),
                            'maxliveprice' => floatval($maxliveprice),
                            'liveid' => $id
                        );
                    ...
                    unset($liveprice,$minliveprice,$maxliveprice);
                }
...
//                    $goods[$key]['options'] = pdo_fetchall("SELECT go.id,go.title,go.marketprice,go.specs,lgo.liveprice,lgo.liveid,lgo.optionid FROM ".tablename('ewei_shop_goods_option')." as go
...
//                    WHERE go.uniacid = :uniacid and go.goodsid = :goodsid  ORDER BY go.displayorder DESC,go.id DESC ",array(':uniacid'=>$uniacid,'goodsid'=>$value['id']));
    ...
    function menu_add(){
        ...
        /*if(empty($id)){
            show_json(0,"该直播间不存在或已删除！");
        }
        $room = pdo_fetch("select id,nestable,tabs from ".tablename('ewei_shop_live')." where id = ".$id." and uniacid = ".$uniacid." ");
        if(empty($room)){
            show_json(0,"该直播间不存在或已删除！");
        }*/
        $menu = array(
    ...
    function hasoption(){
        ...
        /*$goods = pdo_fetch("select id,title,marketprice,hasoption,liveprice from ".tablename('ewei_shop_goods')." where id = :goodsid and uniacid = :uniacid ",$params);
        if(!empty($goodsid)){
    ...
    public function console() {
        ...
        /*
        if(strpos($this_url,'https')!==false && strpos($url,'http:')!==false){
