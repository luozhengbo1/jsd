        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,g.costprice,op.costprice as option_costprice from ' . tablename('ewei_shop_order_goods') . ' og '
...
//                . " left join " . tablename('ewei_shop_member_address') . " a on o.addressid = a.id "
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and iscycelbuy = 1 {$status_condition}", $paras);
            $total = $t['count'];
