...
class Comment_EweiShopV2Page extends PluginWebPage{
    ...
    function main() {
        ...
        $list = pdo_fetchall("SELECT  c.*, o.ordersn,g.title,g.thumb FROM " . tablename('ewei_shop_order_comment') . " c  "
            . " left join " . tablename('ewei_shop_goods') . " g on c.goodsid = g.id  "
            . " left join " . tablename('ewei_shop_order') . " o on c.orderid = o.id  "
            . " WHERE 1 {$condition} ORDER BY createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);

        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('ewei_shop_order_comment') . " c  "
            . " left join " . tablename('ewei_shop_goods') . " g on c.goodsid = g.id  "
            . " left join " . tablename('ewei_shop_order') . " o on c.orderid = o.id  "
    ...
    function post() {
        ...
        if ($_W['ispost']) {
            ...
            if($type == 0) {
                ...
                $checked_array = array(0 => '审核通过', 1 => '审核中', 2 => '审核不通过');
