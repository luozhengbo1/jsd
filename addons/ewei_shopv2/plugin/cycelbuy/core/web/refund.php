...
class Refund_EweiShopV2Page extends PluginWebPage{
    ...
    public function detail()
    {
        ...
        if(empty($data['data']['areas'])){  //areas不存在时拼接 lgt 
            $data['data']['areas'] = $data['data']['province'].' '.$data['data']['city'].' '.$data['data']['area'];
        }
        $data['old_address'] = iunserializer( $data['old_address'] );
        $sql = "select g.title,o.price,o.goodsprice,o.discountprice,o.dispatchprice,o.cycelbuy_periodic from ".tablename( 'ewei_shop_order' )." o ".
        ...
        foreach ( $periods as $k => $v ){
            ...
            if(empty($data['isall'])){  //显示修改期数 lgt
                if($v['id'] == $data['cycleid']){
                    $cycelsn=explode( '_' , $v['cycelsn']);
                    $updateNum = $cycelsn[1];  //修改期数
                }
            }
        }
