...
class Apply_EweiShopV2Page extends DividendWebPage  {
    ...
    function main() {
        ...
        $sql = "select a.*, m.nickname,m.avatar,m.realname,m.mobile,m.agentlevel,a.realname as applyrealname,a.sendmoney from " . tablename('ewei_shop_dividend_apply') . " a "
        ...
        if ($status == 3){
            $realmoney_total = (float)pdo_fetchcolumn("select sum(a.realmoney) from " . tablename('ewei_shop_dividend_apply') . " a "
                . " left join " . tablename('ewei_shop_member') . " m on m.id = a.mid"
            ...
            foreach ($list as &$row) {
                ...
                foreach ($orders as &$order) {
                    $goods = pdo_fetchall("SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname from " . tablename('ewei_shop_order_goods') . " og"
                        ...
                        . " where og.uniacid = :uniacid and og.orderid=:orderid  order by og.createtime  desc ", array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
...
//            dump($exportlist);die();
            m('excel')->export($exportlist, array(
                "title" => $applytitle."分红申请数据-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
        ...
        $total = pdo_fetchcolumn("select count(a.id) from" . tablename('ewei_shop_dividend_apply') . " a "
    ...
    protected  function applyData(){
        ...
        $id = intval($_GPC['id']);
        $apply = pdo_fetch('select * from ' . tablename('ewei_shop_dividend_apply') . ' where uniacid=:uniacid and id=:id limit 1'
    ...
    function pay($params = array(), $mine = array()) {
        ...
        if ($apply['type'] < 2) {
            ...
            if (!empty($data['paytype']['commission']) && $apply['type']== 1){
                if(strpos($member['openid'],'sns_wa_')===false){
                    ...
                    pdo_update('ewei_shop_dividend_apply',array(
        ...
        //发送打款处理消息
//        $this->model->sendMessage($member['openid'], array('dividend' => $mdividend, 'type' => $apply_type[$apply['type']]), TM_COMMISSION_PAY);
