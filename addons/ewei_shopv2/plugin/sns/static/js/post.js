define(['core', 'tpl', './face.js'], function (core, tpl, face) {
    var modal = {page: 1, bid: 0, pid: 0, rpid: 0};
    modal.init = function (params) {
        ...
        $('#btnDelete').click(function () {
            FoxUI.confirm('确认删除此话题?', function () {
                core.json('sns/post/delete', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
                    location.href = core.getUrl('sns/board', {id: modal.bid})
                }, true, true)
            })
        });
        $('#btnCheck').click(function () {
            FoxUI.confirm('确认审核此话题?', function () {
                core.json('sns/post/check', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        $('#btnBest').click(function () {
            ...
            FoxUI.confirm('确认' + msg + '?', function () {
                core.json('sns/post/best', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        $('#btnTop').click(function () {
            ...
            FoxUI.confirm('确认' + msg + '?', function () {
                core.json('sns/post/top', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        $('#btnBestAll').click(function () {
            ...
            FoxUI.confirm('确认' + msg + '?', function () {
                core.json('sns/post/allbest', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        $('#btnTopAll').click(function () {
            ...
            FoxUI.confirm('确认' + msg + '?', function () {
                core.json('sns/post/alltop', {bid: modal.bid, pid: modal.pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        face.init({class: '.post-face .item', input: $('#content')});
        ...
        $('#btnComplain').click(function () {
            ...
            core.json('sns/post/checkPost', {postid: modal.postid}, function (ret) {
                if (ret.status > 0) {
                    var result = ret.result, type = '';
                    console.log(result);
    ...
    modal.getList = function () {
        core.json('sns/post/getlist', {page: modal.page, bid: modal.bid, pid: modal.pid}, function (ret) {
    ...
    modal.bindReplyEvents = function () {
        ...
        $('.reply-list .link-check').unbind('click').click(function () {
            ...
            FoxUI.confirm('确认审核此回复?', function () {
                core.json('sns/post/check', {bid: modal.bid, pid: pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
        ...
        $('.reply-list .link-delete').unbind('click').click(function () {
            ...
            FoxUI.confirm('确认删除此回复?', function () {
                core.json('sns/post/delete', {bid: modal.bid, pid: pid}, function (ret) {
                    if (ret.status == 0) {
                        FoxUI.toast.show(ret.result.message);
                        return
                    }
