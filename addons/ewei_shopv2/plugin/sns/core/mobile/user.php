...
class User_EweiShopV2Page extends SnsMobilePage {
	...
	function main() {
		...
		//关注过的版块
		$boards  =pdo_fetchall('select b.id,b.logo,b.title from '.tablename('ewei_shop_sns_board_follow').' f '
		   .' left join '.tablename('ewei_shop_sns_board').' b on f.bid = b.id '
		   .'   where f.uniacid=:uniacid and f.openid=:openid limit 5',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
		$boards = set_medias($boards,'logo');

		$follow = pdo_fetchall('select count(*) as num from '.tablename('ewei_shop_sns_board_follow').' f '
            .' left join '.tablename('ewei_shop_sns_board').' b on f.bid = b.id '
            .'   where f.uniacid=:uniacid and f.openid=:openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));

        $followcount = $follow['0']['num'];

		//最新3 条话题
		$posts  =pdo_fetchall('select p.id,p.images,p.title ,p.views, b.title as boardtitle,b.logo as boardlogo from '.tablename('ewei_shop_sns_post').' p '
			.' left join '.tablename('ewei_shop_sns_board').' b on p.bid = b.id '
			.'   where p.uniacid=:uniacid and p.openid=:openid and pid=0 and deleted=0 and checked=1 order by createtime desc limit 3',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
		foreach($posts as &$r){
		...
		if($openid==$_W['openid']) {
			...
			$replys = pdo_fetchall('select p.id, p.content, p.views,
				...
				. ' left join ' . tablename('ewei_shop_sns_post') . ' parent on p.pid = parent.id '
				. ' left join ' . tablename('ewei_shop_sns_post') . ' rparent on p.rpid = rparent.id '
				. '   where p.uniacid=:uniacid and p.openid=:openid and p.pid>0 and p.deleted=0 and p.checked=1 order by p.createtime desc limit 3', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			foreach ($replys as &$r) {
	...
	function get_boards(){
		...
		$sql = 'select b.id,b.logo,b.title from '.tablename('ewei_shop_sns_board_follow').' f '
			.' left join '.tablename('ewei_shop_sns_board').' b on f.bid = b.id '
			."   where 1 {$condition} ORDER BY f.createtime asc LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select b.id,b.logo,b.title from '.tablename('ewei_shop_sns_board_follow').' f '
			.' left join '.tablename('ewei_shop_sns_board').' b on f.bid = b.id '
	...
	function get_posts(){
		...
		$sql = "select id,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from " . tablename('ewei_shop_sns_post')
			. "  where 1 {$condition} ORDER BY createtime desc,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
	...
	function get_replys(){
		...
		$sql ='select p.id, p.content, p.views,
			...
			.' left join '.tablename('ewei_shop_sns_post').' parent on p.pid = parent.id '
			.' left join '.tablename('ewei_shop_sns_post').' rparent on p.rpid = rparent.id '
			.' left join '.tablename('ewei_shop_sns_board').' b on b.id=p.bid '
			."   where 1 and {$condition} order by p.createtime desc limit " . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
