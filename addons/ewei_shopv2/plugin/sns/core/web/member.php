...
class Member_EweiShopV2Page extends PluginWebPage {
    ...
    function main() {
        ...
        $sql = "select sm.id as sns_id,sm.credit as sns_credit, sm.level as sns_level,sm.createtime,sm.isblack,sm.openid, "
             ." dm.id,dm.nickname,dm.avatar,dm.agentid, dm.isagent,dm.status,dm.realname,dm.mobile,"
             ." a.nickname as agentnickname,a.avatar as agentavatar,"
             ." f.follow as followed from " . tablename('ewei_shop_sns_member') . " sm "
            . " left join " . tablename('ewei_shop_member') . " dm on dm.openid = sm.openid and dm.uniacid = sm.uniacid"
            . " left join " . tablename('ewei_shop_member') . " a on a.id=dm.agentid"
            . " left join " . tablename('ewei_shop_sns_level') . " l on sm.level =l.id"
            . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}"
            . " where 1 {$condition}  ORDER BY sm.id DESC";
        ...
        $total = pdo_fetchcolumn("select count(*) from" . tablename('ewei_shop_sns_member') . " sm "
            . " left join " . tablename('ewei_shop_member') . " dm on dm.openid = sm.openid and dm.uniacid = sm.uniacid"
            . " left join " . tablename('ewei_shop_member') . " a on a.id=dm.agentid"
            . " left join " . tablename('ewei_shop_sns_level') . " l on sm.level =l.id"
            . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}"
