...
class Complain_EweiShopV2Page extends PluginWebPage {
	...
	function main() {
		...
		$type = trim($_GPC['type']);
		if($type == 'processed'){//已审核
			$condition .= " and checked = 1 and deleted = 0 ";
		}else if($type == 'untreated'){//待审核
			$condition .= " and checked = 0 and deleted = 0 ";
		}else if($type == 'cancel') {//已取消
			$condition .= " and checked = -1 and deleted = 0 ";
		}else if($type == 'deleted') {//已取消
	 ...
	 * 投诉消息发送
	 * */
	function sendComMessage($complainid,$type = true){
		global $_W, $_GPC;
		$uniacid = $_W['uniacid'];
	...
	function category(){
		...
		if (!empty($_GPC['catid'])) {
			foreach ($_GPC['catid'] as $key => $value) {
				$data = array(
					'name' => trim($_GPC['catname'][$key]),
					'displayorder' => $key,
					'status' => intval($_GPC['status'][$key]),
					'uniacid' => $_W['uniacid']
				);
				if (empty($value)) {
