...
class Manage_EweiShopV2Page extends PluginWebPage {
    ...
    function main() {
        ...
        $list = pdo_fetchall("SELECT s.*,b.title,m.avatar,m.nickname,m.realname,m.mobile  FROM " . tablename('ewei_shop_sns_manage') . " s "
          ." left join ".tablename('ewei_shop_sns_board').' b on s.bid= b.id and s.uniacid = b.uniacid '
          ." left join ".tablename('ewei_shop_member').' m on s.openid = m.openid and s.uniacid = m.uniacid '
          ." WHERE 1 {$condition}  ORDER BY s.id  DESC limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn("SELECT count(1) FROM " . tablename('ewei_shop_sns_manage')  ." s "
            ." left join ".tablename('ewei_shop_sns_board').' b on s.bid= b.id and s.uniacid = b.uniacid '
            ." left join ".tablename('ewei_shop_member').' m on s.openid = m.openid and s.uniacid = m.uniacid '
