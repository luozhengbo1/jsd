...
class Replys_EweiShopV2Page extends PluginWebPage
{
    ...
    function main()
    {
        ...
        if(!empty($pid)){
            ...
            $member['postcount'] = pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_sns_post').' where uniacid=:uniacid and openid=:openid and pid=0 and deleted=0 limit 1'
                ,array(':uniacid'=>$_W['uniacid'],':openid'=>$member['openid']));

            $member['replycount'] = pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_sns_post').' where uniacid=:uniacid and openid=:openid and pid>0 and deleted=0  limit 1'
                ,array(':uniacid'=>$_W['uniacid'] ,':openid'=>$member['openid']));
        ...
        $sql = "select id,bid,pid, rpid,title,createtime,content,images ,openid, nickname,avatar,checked,deleted from " . tablename('ewei_shop_sns_post')
            . "  where 1 {$condition} ORDER BY createtime asc LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        ...
        foreach ($list as $key=> &$row) {
            ...
            $row['member'] = array(
                'id'=>$rmember['id'],
                'sns_credit'=>$rmember['sns_credit'],

                'postcount'=> pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_sns_post').' where uniacid=:uniacid and openid=:openid and pid=0 and deleted=0 limit 1'
                    ,array(':uniacid'=>$_W['uniacid'],':openid'=>$rmember['openid'])),

                'replycount'=>pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_sns_post').' where uniacid=:uniacid and openid=:openid and pid>0 and deleted=0 limit 1'
                ,array(':uniacid'=>$_W['uniacid'],':openid'=>$rmember['openid'])),
            );
