    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            $data = array(
                ...
                'noimage' => intval($_GPC['noimage']),

                'showlevels' => is_array($_GPC['showlevels']) ? implode(",", $_GPC['showlevels']) : '',
                'postlevels' => is_array($_GPC['postlevels']) ? implode(",", $_GPC['postlevels']) : '',
                'showgroups' => is_array($_GPC['showgroups']) ? implode(",", $_GPC['showgroups']) : '',
                'postgroups' => is_array($_GPC['postgroups']) ? implode(",", $_GPC['postgroups']) : '',


                'postcredit' => intval($_GPC['postcredit']),
                'replycredit' => intval($_GPC['replycredit']),
                'topcredit' => intval($_GPC['topcredit']),
                'topboardcredit' => intval($_GPC['topboardcredit']),
                'bestcredit' => intval($_GPC['bestcredit']),
                'bestboardcredit' => intval($_GPC['bestboardcredit']),

                'needfollow' => intval($_GPC['needfollow']),
                'needpostfollow' => intval($_GPC['needpostfollow']),
                ...
                'needcheck'=>intval($_GPC['needcheck']),
                'needcheckmanager'=>intval($_GPC['needcheckmanager']),


                'needcheckreply'=>intval($_GPC['needcheckreply']),
                'needcheckreplymanager'=>intval($_GPC['needcheckreplymanager']),


                'showsnslevels' => is_array($_GPC['showsnslevels']) ? implode(",", $_GPC['showsnslevels']) : '',
                'postsnslevels' => is_array($_GPC['postsnslevels']) ? implode(",", $_GPC['postsnslevels']) : '',
        ...
        if(!empty($item)) {
            $managers = pdo_fetchall("select m.id,m.openid,m.avatar,m.nickname from " . tablename('ewei_shop_sns_manage') . " sm "
                . ' left join ' . tablename('ewei_shop_member') . " m on sm.openid = m.openid and sm.uniacid = m.uniacid"
                . " where sm.uniacid=:uniacid and sm.bid=:bid ", array(':uniacid' => $_W['uniacid'], ':bid' => $item['id']));
        }
        include $this->template();
    }
