...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    function get_list()
    {
        ...
        foreach ($list as &$row) {
            ...
            //所有商品
            $sql = "SELECT og.goodsid,og.total,g.title,g.thumb,g.type, og.price,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,g.status FROM " . tablename('ewei_shop_order_goods') . " og "
...
//            print_r($row['goods']);exit;
    ...
    function detail()
    {
        ...
        $goods = pdo_fetchall("select g.id,g.type, og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.storeids{$diyformfields}  from " . tablename('ewei_shop_order_goods') . " og "
...
//        }
    ...
    function cycel_orderList(){
        ...
        foreach ($list as &$row) {
            ...
            //所有商品
            $sql = "SELECT og.goodsid,og.total,g.title,g.thumb,og.price,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,g.status FROM " . tablename('ewei_shop_order_goods') . " og "
                . " left join " . tablename('ewei_shop_goods') . " g on og.goodsid = g.id "
                . " left join " . tablename('ewei_shop_goods_option') . " op on og.optionid = op.id "
                . " where $scondition order by og.id asc";
