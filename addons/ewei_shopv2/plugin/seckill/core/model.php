...
class SeckillModel extends PluginModel
{
    ...
    function setTaskCache($id)
    {
        ...
        foreach ($alltimes as &$time) {
            ...
            //            缓存商品的库存信息,不在次做商品缓存.在添加任务时做.
//            foreach($goods as $key => $val){
//                $goodskey ="{$_W['uniacid']}_seckill_stock_{$val['taskid']}_{$val['roomid']}_{$val['timeid']}_{$val['goodsid']}_{$val['optionid']}";
...
//
        ...
        if (!empty($timegoods)) {
            ...
            //是否允许过期抢购
            if($currenttime>$timegoods[0]['end_time']){
                if(!$roominfo['oldshow']){
                 return false;
                }
            }
