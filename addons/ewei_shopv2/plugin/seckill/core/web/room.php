            ...
            foreach($times as $time) {
                ...
                if (empty($open)) {
                    ...
                    if (is_array($timegoods)) {
                        ...
                        //新商品ID，判断同一时间段是否有重复商品
                        $goodsids = array();
                        foreach ($timegoods as $k => $v) {
                             $count = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_seckill_task_goods') . ' where taskid=:taskid and roomid=:roomid  and goodsid=:goodsid  limit 1',
                                    array(':taskid' => $taskid, ':roomid'=>$roomid, ':goodsid' => $k));
                             if($count<=0){
                                   $goodsids[] = $k;
                             }
                            if (empty($v)) {
                        ...
                        foreach ($timegoods as $k => $v) {
                            if (empty($v)) {
                                ...
                                $goods = pdo_fetch('select * from ' . tablename('ewei_shop_seckill_task_goods') . ' where taskid=:taskid and roomid=:roomid and timeid=:timeid and goodsid=:goodsid  limit 1',
                                    array(':taskid' => $taskid, ':roomid'=>$roomid, ':timeid' => $timeid, ':goodsid' => $k));
                                if (empty($goods)) {
                                ...
                                foreach ($optionids as $option) {
                                    ...
                                    $goods = pdo_fetch('select * from ' . tablename('ewei_shop_seckill_task_goods') . ' where taskid=:taskid and roomid=:roomid and timeid=:timeid and goodsid=:goodsid and optionid=:optionid limit 1',
                                        array(':taskid' => $taskid, ':roomid'=>$roomid, ':timeid' => $timeid, ':goodsid' => $k, ':optionid' => $option));
                                    if (empty($goods)) {
            ...
            foreach ($times as &$t) {
                ...
                foreach ($goods as &$g) {
                    ...
                    if ($g['hasoption']) {
                        ...
                        $g['optiontitle'] = pdo_fetchall("select tg.id,tg.goodsid,tg.optionid,tg.price as packageprice,tg.maxbuy,tg.totalmaxbuy, g.title,g.marketprice,tg.commission1,tg.commission2,tg.commission3,tg.total from " . tablename('ewei_shop_seckill_task_goods') . "  tg  left join " . tablename('ewei_shop_goods') . " g on tg.goodsid = g.id  where tg.roomid=:roomid and tg.timeid=:timeid and tg.taskid=:taskid and tg.timeid=:timeid  and tg.goodsid=:goodsid and  tg.uniacid =:uniacid ",
                            array(':timeid' => $t['id'], ':taskid' => $item['taskid'],':roomid'=>$roomid, ':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
