...
//                                        获取商品佣金
        ...
        where g.id = ".$good['id']." and g.hasoption=1";
                                                        ...
                                                        $goods_price = pdo_fetch("select minprice,maxprice,ispresell,presellprice from " . tablename('ewei_shop_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $good['id'], ':uniacid' => $_W['uniacid']));
                                                        if( $good['productprice'] < $goods_price['maxprice'] ){
                                                            $good['productprice'] = (float)$goods_price['maxprice'];
                                                        }

                                                        // 如果预售开启并且比最低价格低,那么就修改最低价格为预售价格 #[5903]
                                                        if ($goods_price['ispresell'] == 1 && $goods_price['minprice'] > $goods_price['presellprice']) {
