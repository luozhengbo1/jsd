...
class User_EweiShopV2Page extends MerchWebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT u.*,r.rolename FROM " . tablename('ewei_shop_merch_account') . " u  "
		...
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('ewei_shop_merch_account') . " u  "
			. " left join " . tablename('ewei_shop_merch_perm_role') . " r on u.roleid =r.id  "
...
//			$role = pdo_fetch('select id,rolename,merchid from ' . tablename('ewei_shop_merch_perm_role') . ' where uniacid=:uniacid and deleted=0', array(':uniacid' => $_W['uniacid']));
            ...
            $item = pdo_fetch('select u.*,r.rolename,r.merchid from ' . tablename('ewei_shop_merch_account') . ' u '
                . ' left join ' . tablename('ewei_shop_merch_perm_role') . ' r on u.roleid = r.id '
                . ' where u.id=:id AND u.uniacid=:uniacid AND u.merchid=:merchid AND r.uniacid=:uniacid AND r.deleted=0 ', array(':id'=>$id,':uniacid'=>$_W['uniacid'],':merchid'=>$_W['merchid']));
