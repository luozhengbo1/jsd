...
class Log_EweiShopV2Page extends MerchWebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT  log.* ,u.username FROM " . tablename('ewei_shop_merch_perm_log') . " log  "
			. " left join " . tablename('ewei_shop_merch_account') . " u on log.uid = u.id and log.merchid = u.merchid and log.uniacid = u.uniacid"
			. " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);

		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('ewei_shop_merch_perm_log') . " log  "
			. " left join " . tablename('ewei_shop_merch_account') . " u on log.uid = u.id and log.merchid = u.merchid and log.uniacid = u.uniacid"
