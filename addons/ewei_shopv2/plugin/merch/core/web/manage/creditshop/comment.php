...
class Comment_EweiShopV2Page extends MerchWebPage {
    ...
    protected function getList($checked=0){
        ...
        $list = pdo_fetchall("SELECT  c.*,g.title,g.thumb FROM " . tablename('ewei_shop_creditshop_comment') . " c  "
            . " left join " . tablename('ewei_shop_creditshop_goods') . " g on c.goodsid = g.id  "
            . " WHERE 1 {$condition} ORDER BY `time` desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);

        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('ewei_shop_creditshop_comment') . " c  "
            . " left join " . tablename('ewei_shop_creditshop_goods') . " g on c.goodsid = g.id  "
    ...
    protected function post(){
        ...
        if($_W['ispost']){
            if(!empty($item) && empty($item['virtual'])){
                // 回复
                $arr  = array(
                    'reply_content' => trim($_GPC['reply_content']),
                    'append_reply_content' => trim($_GPC['append_reply_content']),
                    'checked' => intval($_GPC['checked']),
                    'append_checked' => intval($_GPC['append_checked']),
                    'reply_images' => is_array($_GPC['reply_images']) ? iserializer(m('common')->array_images($_GPC['reply_images'])) : iserializer(array()),
                    'append_reply_images' => is_array($_GPC['append_reply_images']) ? iserializer(m('common')->array_images($_GPC['append_reply_images'])) : iserializer(array()),
                    'reply_time' => time(),
                    'append_reply_time' => !empty($_GPC['append_reply_content']) || !empty($_GPC['append_reply_images']) ? time() : 0
                );
                ...
                $arr = array(
                    ...
                    'images' => is_array($_GPC['images']) ? iserializer(m('common')->array_images($_GPC['images'])) : iserializer(array()),
                    'time' => !empty($_GPC['time'])?strtotime($_GPC['time']):time(),
                    'reply_content' => trim($_GPC['reply_content']),
                    'reply_images' => is_array($_GPC['reply_images']) ? iserializer(m('common')->array_images($_GPC['reply_images'])) : iserializer(array()),
                    'reply_time' => !empty($_GPC['reply_time'])?strtotime($_GPC['reply_time']):time(),
                    'append_content' => trim($_GPC['append_content']),
                    'append_images' => is_array($_GPC['append_images']) ? iserializer(m('common')->array_images($_GPC['append_images'])) : iserializer(array()),
                    'append_time' => !empty($_GPC['append_time'])?strtotime($_GPC['append_time']):time(),
                    'append_reply_content' => trim($_GPC['append_reply_content']),
                    'append_reply_images' => is_array($_GPC['append_reply_images']) ? iserializer(m('common')->array_images($_GPC['append_reply_images'])) : iserializer(array()),
                    'append_reply_time' => !empty($_GPC['append_reply_time'])?strtotime($_GPC['append_reply_time']):time()
                );
    ...
    public function delete(){
        ...
        foreach ($items as $item) {
            ...
            $goods = pdo_fetch('select id,thumb,title from ' . tablename('ewei_shop_creditshop_goods') . ' where id=:id and merchid = '.$_W['merchid'].' and uniacid=:uniacid limit 1',
                array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
