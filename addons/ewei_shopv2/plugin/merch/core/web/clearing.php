...
class Clearing_EweiShopV2Page extends PluginWebPage {
    ...
    protected function main($status = 0)
    {
        ...
        $sql = "select u.merchname,u.mobile,u.realname,u.logo,o.* from " . tablename('ewei_shop_merch_clearing') . " o "
...
//导出Excel
        ...
        $total = pdo_fetchcolumn("select COUNT(u.id) from " . tablename('ewei_shop_merch_user') . " u "
    ...
    protected function post()
    {
        ...
        if ($_W['ispost'])
        {
            ...
            $data = array(
                'merchid' => $merchid,
                'clearno' => empty($_GPC['clearno']) ? m('common')->createNO('merch_clearing', 'clearno', 'MB') : $_GPC['clearno'],
                'uniacid' => $_W['uniacid'],
                'goodsprice' => floatval($_GPC['goodsprice']),
                'dispatchprice' => floatval($_GPC['dispatchprice']),
                'deductprice' => floatval($_GPC['deductprice']),
                'deductcredit2' => floatval($_GPC['deductcredit2']),
                'discountprice' => floatval($_GPC['discountprice']),
                'isdiscountprice' => floatval($_GPC['isdiscountprice']),
                'price' => floatval($_GPC['price']),
                'realprice' => floatval($_GPC['realprice']),
                'payrate' => floatval($_GPC['payrate']),
                'realpricerate' => floatval($_GPC['realpricerate']),
                'deductenough' => floatval($_GPC['deductenough']),
                'merchdeductenough' => floatval($_GPC['merchdeductenough']),
                'merchcouponprice' => floatval($_GPC['merchcouponprice']),
            ...
            ($data['goodsprice']+$data['dispatchprice']) > $data['realprice'] ? $data['realprice'] = ($data['goodsprice']+$data['dispatchprice']-$data['merchdeductenough']-$data['merchcouponprice']) : $data['realprice'];
            if ($data['status'] == 2){
