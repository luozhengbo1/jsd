...
class Index_EweiShopV2Page extends MmanageMobilePage {
    ...
    public function main(){
        ...
        elseif($type==1 && !cv('finance.log.withdraw')){
            $this->message("您没有查看提现申请的权限");
        }
        elseif($type==2 && !cv('finance.credit.credit1')){
            $this->message("您没有查看积分明细的权限");
        }
        elseif($type==3 && !cv('finance.credit.credit2')){
    ...
    protected function log($type) {
        ...
        $sql = "select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.apppay from " . tablename('ewei_shop_member_log') . " log "
