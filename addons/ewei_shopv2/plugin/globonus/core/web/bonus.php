...
class Bonus_EweiShopV2Page extends PluginWebPage
{
    ...
    protected function get_list($status = 0)
    {
        ...
        //结算单号 	日期 	订单信息 	股东数 	预计分红 	最终分红 	状态 	操作
    ...
    function detail()
    {
        ...
        $sql = "select b.*, m.nickname,m.avatar,m.realname,m.weixin,m.mobile,l.levelname,b.bonus,m.partnerlevel,m.id as mid from " . tablename('ewei_shop_globonus_billp') . " b "
    ...
    function payp()
    {
        ...
        if (!empty($member)) {
            ...
            if (is_error($result)) {
                ...
                //打款通知
                $this->model->sendMessage($partner['openid'], array(
