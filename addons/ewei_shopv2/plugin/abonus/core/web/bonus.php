...
class Bonus_EweiShopV2Page extends PluginWebPage
{
    ...
    protected function get_list($status = 0)
    {
        ...
        //结算单号 	日期 	订单信息 	区域代理数 	预计分红 	最终分红 	状态 	操作
    ...
    function detail()
    {
        ...
        $sql = "select b.*, m.nickname,m.avatar,m.realname,m.weixin,m.mobile,l.levelname,b.bonus1,b.bonus2,b.bonus3,m.aagentlevel,m.aagenttype,m.id as mid from " . tablename('ewei_shop_abonus_billp') . " b "
            . " left join " . tablename('ewei_shop_member') . " m on m.openid = b.openid and m.uniacid = b.uniacid"
            . " left join " . tablename('ewei_shop_abonus_level') . " l on l.id = m.aagentlevel"
            . " where 1 {$condition} ORDER BY status asc ";
    ...
    function build()
    {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($data['aagents'] as $a) {
                ...
                if ($a['bonusmoney_send1'] * $rate1 > 0 || $a['bonusmoney_send2'] * $rate2 > 0 || $a['bonusmoney_send3'] * $rate3 > 0) {
    ...
    function payp()
    {
        ...
        if (!empty($member)) {
            ...
            if (is_error($result)) {
                ...
                $this->model->upgradeLevelByBonus($aagent['openid']);
                    ...
                    //打款通知
                 $this->model->sendMessage($aagent['openid'], array(
        ...
        if (is_error($result)) {
            show_json(0, array(
                ...
                'aagentcount11' => $aagentcount11,
                'aagentcount22' => $aagentcount22,
                'aagentcount33' => $aagentcount33,
                'aagentcount44' => $aagentcount44,
            ...
            show_json(1, array(
                ...
                'aagentcount22' => $aagentcount22,
                'aagentcount33' => $aagentcount33,
                'aagentcount44' => $aagentcount44, 'full' => $full));
        }
    }

    function paymoney_level()
    {
        global $_W, $_GPC;
        ...
        $ret =  array(
            ...
            'bonusmoney_pay1' => $totalmoney['paymoney1'],
            'bonusmoney_pay2' => $totalmoney['paymoney2'],
            'bonusmoney_pay3' => $totalmoney['paymoney3']);

        pdo_update('ewei_shop_abonus_bill',$ret,array("id" => $id, 'uniacid' => $_W['uniacid']));
        show_json(1,$ret);
    }

    function paymoney_aagent()
    {
        global $_W, $_GPC;
        ...
        if ($paymoneytype <= 0 || $paymoneytype > 3) {
            show_json(0, '参数错误!');
        }
