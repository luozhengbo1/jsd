...
//导出Excel
    ...
    protected function post()
    {
        ...
        if (!empty($_W['cashieruser']['merchid'])){
            $total_money = (float)pdo_fetchcolumn("select sum(money-orderprice+deduction) from " . tablename('ewei_shop_cashier_pay_log') .  " where uniacid=:uniacid {$condition}{$payday_sql}", $params);
        }else{
            $total_money = (float)pdo_fetchcolumn("select sum(money+deduction) from " . tablename('ewei_shop_cashier_pay_log') .  " where uniacid=:uniacid {$condition}{$payday_sql}", $params);
        }

        $withdraw = empty($userinfo['withdraw']) ? 0 : (float)$userinfo['withdraw'];
        ...
        if ($_W['ispost'])
        {
            ...
            $data = array(
                'cashierid' => $cashierid,
                'uniacid' => $_W['uniacid'],
                'clearno' => empty($_GPC['clearno']) ? m('common')->createNO('cashier_clearing', 'clearno', 'MB') : $_GPC['clearno'],
                'status' => 0,
                'money' => $money,
            ...
            if (!$id)
            {
                ...
                $this->model->sendMessage(array(
                    'name'=>!empty($clearing['name'])?$clearing['name']:$userinfo['name'],
                    'mobile'=>!empty($clearing['mobile'])?$clearing['mobile']:$userinfo['mobile'],
                    'money'=>$data['money'],
                    'createtime'=>time()
                ),'apply_clearing');
            }
