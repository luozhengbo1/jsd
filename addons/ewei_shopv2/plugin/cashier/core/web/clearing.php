...
//导出Excel
    ...
    protected function post()
    {
        ...
        if ($_W['ispost'])
        {
            ...
            $money = (float)$_GPC['realmoney'];
            if (empty($money)){
                $money = (float)$clearing['money'];
            }
            ...
            $this->model->sendMessage(array(
                'name'=>$clearing['name'],
                'mobile'=>$clearing['mobile'],
                'money'=>$clearing['money'],
                'realmoney'=>$clearing['realmoney'],
                'createtime'=>$clearing['createtime'],
                'paytime'=>time()
            ),'apply_clearing',$clearing['openid']);
            show_json(1);
        }
        ...
        if (!empty($clearing['merchid'])){
            $total_money = (float)pdo_fetchcolumn("select sum(money-orderprice+deduction) from " . tablename('ewei_shop_cashier_pay_log') .  " where uniacid=:uniacid {$condition}", $params);
        }else{
