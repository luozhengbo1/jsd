...
class Couponpay_EweiShopV2Page extends CashierMobilePage {
    ...
    public function pay($params = array(), $mine = array())
    {
        ...
        $this->log = $this->model->goodsCalculate(array(),$goods,array(
            'paytype'=>$paytype,
            'openid'=>$_W['openid'] != 'alipay' ? $_W['openid'] : '',
            'money'=>$paymoney,
            'couponpay'=>$couponpaymoney,
            'nosalemoney'=>$no_money,
            'deduction'=>0,
            'mobile'=>0,
            'title'=>$title,
