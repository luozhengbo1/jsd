...
class CashierModel extends PluginModel
{
    ...
    public function createOrder(array $array,$return = null,$can_sale=true)
    {
        ...
        $data = array(
            ...
            'coupon'=>isset($array['coupon']) ? $array['coupon'] : 0,//是否有赠送的优惠卷,优惠券ID
            'usecoupon'=>isset($array['usecoupon']) ? $array['usecoupon'] : 0,//使用的优惠券
            'usecouponprice'=>isset($usecoupon['money']) ? $usecoupon['money'] : 0,//使用优惠券 减免的价格
            'status'=>0
        );
    ...
    public function pay($data,$auth_code=null)
    {
        ...
        if ($auth_code!==null){
            ...
            //获取openid 如果paytype不等于1的话
            //m('common')->authCodeToOpenid($auth_code,$this->model->wechayPayInfo($_W['cashieruser']));die;
        }
        if ($data['paytype'] == 0){
    ...
    public function refund($id)
    {
        ...
        if (com('coupon') && !empty($pay_log['usecoupon'])) {
            com('coupon')->returnConsumeCoupon($pay_log['usecoupon']); //申请退款成功
        }
    ...
    public function getEnoughs($price = 0) {
        ...
        if (isset($set['enoughtime']) && !$this->sale_time($set['enoughtime'])){
            return array(
                'old_price' => $price,//传过来的金额
                'new_price' => $price,//返回的新金额
                'enough' => 0,//满多少钱
                'money' => 0,//减多少钱
            );
        }
        ...
        return array(
            'old_price' => $price,//传过来的金额
            'new_price' => round($new_price,2),//返回的新金额
            'enough' => $enough,//满多少钱
            'money' => $money,//减多少钱
        );
    }
    ...
    public function getDiscount($price = 0)
    {
        ...
        return array(
            'old_price' => $price,//传过来的金额
            'new_price' => round($new_price,2),//返回的新金额
            'discount' => $set['discount'],//打折折扣
            'money' => round($price - $new_price,2),//折扣的钱
        );
    }
    ...
    public function createGoodsOrder($goods,$openid='',$setmoney=null)
    {
        ...
        $order['grprice'] = $goodsprice; //商品实际价格
        $order['cash'] = 0;
        $order['status'] = 0;
        $order['remark'] = trim($_GPC['remark']);
        $order['addressid'] = 0;
        $order['goodsprice'] = $goodsprice;//商品总价
        $order['storeid'] = 0;//门店id
...
//        print_r("money-".$params['money']."  ");
//        print_r("orderprice-".$order['orderprice'] ."  ");
//        print_r("goodsprice-".$order['goodsprice'] ."  ");

//        if ($params['money'] == $order['orderprice']){
...
//        return array('res'=>error('-101','支付金额与商品金额有误差!'));
    ...
    public function setStocks($orderid = 0,$manageopenid='')
    {
        ...
        foreach ($goods as $g) {
            $stocktype = -1; //0 不设置库存情况 -1 减少 1 增加
            if (!empty($stocktype)) {
    ...
    function caculatecoupon($couponid, $totalprice,$openid=null)
    {
        ...
        return array(
            'old_price' => $totalprice,//传过来的金额
            'new_price' => round($new_price,2),//返回的新金额
            'discount' => $discount,//打折折扣
            'money' => $deductprice,//折扣的钱
        );
    }
