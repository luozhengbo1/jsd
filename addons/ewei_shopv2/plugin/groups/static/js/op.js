define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.cancel = function (id, remark) {
        core.json('groups/orders/cancel', {id: id, remark: remark}, function (pay_json) {
    ...
    modal.delete = function (id) {
        core.json('groups/orders/delete', {id: id}, function (pay_json) {
            if (pay_json.status == 1) {
                if (modal.fromDetail) {
                    location.href = core.getUrl('groups/orders')
                } else {
    ...
    modal.finish = function (id) {
        core.json('groups/orders/finish', {id: id}, function (pay_json) {
            if (pay_json.status == 1) {
                location.reload();
                return
            }
