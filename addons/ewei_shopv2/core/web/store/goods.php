            ...
            foreach($list as $goods)
            {
                ...
                if(empty($count))
                {
                    ...
                    $data=array(
                        ...
                        "storeid"=>$id,
                        "goodsid"=>$goods['id'],
                        "sdowpayment"=>$goods['dowpayment'],
                        "smarketprice"=>$goods['marketprice'],
                        "sminprice"=>$goods['marketprice'],
                        "smaxprice"=>$goods['marketprice'],
                        "stotal"=>$goods['total'],
                        "createtime"=>time(),
                        "deleted"=>0
                    );
...
//        $goodsgroup = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_store_goodsgroup')." WHERE uniacid = :uniacid", array(':uniacid'=>$_W['uniacid']));
        include $this->template();
    }
    ...
    function goodsoption(){
        ...
        if ($_W['ispost']) {
            ...
            if (is_array($price)){
                if (is_array($price)) {
                    $max = array_search(max($price), $price);//价格最大值
                    $min = array_search(min($price), $price);//价格最小值
