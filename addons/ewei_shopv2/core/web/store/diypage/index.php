...
class Index_EweiShopV2Page extends WebPage {
    ...
    public function settings() {
        ...
        if($_W['ispost']){
            ...
            if($type=='pageset'){
                ...
                // $this->updateList($storeid);
            }
            exit();
        }

        $item = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_store'). 'WHERE uniacid=:uniacid AND id=:id LIMIT 1', array(
            ':uniacid'=>$_W['uniacid'],
            ':id'=>$storeid
        ));
        if(empty($item)){
        ...
        // 读取模板列表
        $templist = pdo_fetchall('SELECT * FROM '. tablename('ewei_shop_newstore_diypage_temp'). 'WHERE (uniacid=:uniacid OR uniacid=0) AND status>0 ORDER BY uniacid asc', array(
            ':uniacid'=>$_W['uniacid']
        ));

        // 处理可用模板
        /*
        if(!empty($templist) && !empty($item['diypage_list'])){
            $diypage_list = explode(',', $item['diypage_list']);
            $diypage_list= array_unique($diypage_list);
            foreach ($templist as &$tempitem){
                if(in_array($tempitem['id'], $diypage_list) || $tempitem['uniacid']==0){
                    $tempitem['selected'] = 1;
                }
            }
            unset($tempitem);
        ...
        // 读取页面列表
        $pagelist = pdo_fetchall('SELECT * FROM '. tablename('ewei_shop_newstore_diypage'). 'WHERE uniacid=:uniacid AND storeid=:storeid AND status=1', array(
            ':uniacid'=>$_W['uniacid'],
            ':storeid'=>$storeid
        ));
    ...
    public function create() {
        ...
        // 读取模板列表
        $list = pdo_fetchall('SELECT * FROM '. tablename('ewei_shop_newstore_diypage_temp'). 'WHERE (uniacid=:uniacid OR uniacid=0) AND status>0', array(
            ':uniacid'=>$_W['uniacid']
        ));
    ...
    protected function updatePage($storeid) {
        ...
        // 更新数据
        pdo_update('ewei_shop_store', array(
            'diypage'=>$diypage,
            'diypage_ispage'=>$diypage_ispage
        ), array(
            'uniacid'=>$_W['uniacid'],
            'id'=>$storeid
        ));
    ...
    public function goods_query(){
        ...
        if(empty($storeid)){
            ...
            $list = pdo_fetchall('SELECT g.id, g.title, g.subtitle, g.thumb, ng.sminprice, g.productprice, g.total, g.sales FROM '. tablename('ewei_shop_goods'). " g LEFT JOIN ". tablename('ewei_shop_newstore_goods').
                " ng ON ng.goodsid=g.id WHERE 1 {$condition} order by g.createtime desc", $params);
        }
    ...
    protected function post() {
        ...
        $initJson = json_encode(array(
            'id'=>$id,
            'data'=>$item['data'],
            'storeid'=>0,
            'type'=>0,
            'attachurl'=>$_W['attachurl'],
            'newstore'=>$_W['plugin']=='newstore'
        ));
