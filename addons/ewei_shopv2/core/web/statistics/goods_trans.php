...
class Goods_trans_EweiShopV2Page extends WebPage {
	...
	function main() {
		...
		$sql = "SELECT g.id,g.title,g.thumb,g.viewcount,"
		         . "(select sum(og.total) from  " . tablename('ewei_shop_order_goods') . " og left join ".tablename('ewei_shop_order')." o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as buycount"
