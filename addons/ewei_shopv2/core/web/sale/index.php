...
class Index_EweiShopV2Page extends ComWebPage {
    ...
    function main() { 
        ...
        //积分优惠
        elseif(cv('sale.credit1')){
            header('location: '.webUrl('sale/credit1'));
        }
        //套餐管理
        elseif(cv('sale.package')){
        ...
        //全返管理
        elseif(cv('sale.fullback')){
            header('location: '.webUrl('sale/fullback'));
        }
        //找人代付
        elseif(cv('sale.peerpay')){
        ...
        //微信卡券
        elseif(cv('sale.wxcard')){
            header('location: '.webUrl('sale/wxcard'));
        }
        //关注回复
        elseif(cv('sale.virtual')){
    ...
    public function credit1()
    {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($postenough1 as $key => $value) {
                ...
                if ($enough > 0) {
                    $enough1[] = array(
                        'enough1_1' => floatval($_GPC['enough1_1'][$key]),
                        'enough1_2' => floatval($_GPC['enough1_2'][$key]),
                        'give1' => intval($_GPC['give1'][$key]),
                    );
                }
            }
            ...
            foreach ($postenough2 as $key => $value) {
                ...
                if ($enough > 0) {
                    $enough2[] = array(
                        'enough2_1' => floatval($_GPC['enough2_1'][$key]),
                        'enough2_2' => floatval($_GPC['enough2_2'][$key]),
                        'give2' => intval($_GPC['give2'][$key])
                    );
                }
            }
...
//        dump(com_run('sale::getCredit1','o6tC-wmZovDTswVba3Kg1oAV_dd0','80.00',1,1));
        $data = m('common')->getPluginset('sale');
