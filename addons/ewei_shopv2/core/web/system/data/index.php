			...
			if (is_array($_GPC['shop'])) {
				foreach ($_GPC['shop'] as $data) {
					if ($data == 'goods') {
						...
						/*if ($wechatid != -1) {
							$members = pdo_fetchall('select id,uid,openid,uniacid from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and credit1 > 0 ', array(':uniacid' => $wechatid));
						} else {
							$members = pdo_fetchall('select id,uid,openid,uniacid from ' . tablename('ewei_shop_member').' where credit1 > 0 ');
						}
