...
//            print_r($sql);die();
                ...
                if(!empty($m)){
                    ...
                    if(!empty($member['credit1'])){
                        if(empty($m['credit1'])){
                            ...
                            $credit1=$member['credit1']>$m['credit1']?$member['credit1']:$m['credit1'];
                        }
                    ...
                    if(!empty($member['credit2'])){
                        if(empty($m['credit2'])){
                            ...
                            $credit2=$member['credit2']>$m['credit2']?$member['credit2']:$m['credit2'];
                        }
    ...
    /*    function upmember(){
            ...
            foreach($members as $member) {
                ...
                if (!empty($uid)) {
                    //如果已关注
                    $mc_member= pdo_fetch("SELECT *  FROM " . tablename('mc_members') . " WHERE `uid` = :uid limit 1", array(':uid' => $uid));

                    if(!empty($mc_member)){
                        pdo_update('ewei_shop_member', array('nickname' => $mc_member['nickname'],'realname'=>$mc_member['realname'],'avatar'=>$mc_member['avatar']), array('id' => $member['id']));
                    }
                }
            }
            show_json(1);
