...
//        $did = array();
    ...
    function sharePage(){
        ...
        foreach ($firendlist as $fk => $fv) {
            ...
            $firendlist[$fk]['nickname'] = $member['nickname'];
            $firendlist[$fk]['text'] = $this -> textClause($fv['textkey']);
//            $groupSql = 'SELECT * FROM '.tablename('ewei_shop_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND openid = "'.$fv['openid'].'" AND shareident = "'.$fv['shareident'].'" AND id <> '.intval($fv['did']) ;
//            $firendlist[$fk]['group'] = pdo_fetchall($groupSql);
//            foreach($firendlist[$fk]['group'] as $gk => $gv){
//                $firendlist[$fk]['group'][$gk]['headimg'] = $member['avatar'];
//                $firendlist[$fk]['group'][$gk]['nickname'] = $member['nickname'];
//                $firendlist[$fk]['group'][$gk]['text'] = $this -> textClause($gv['textkey']);
//            }
        }
        $firendlist = array_reverse($firendlist);
