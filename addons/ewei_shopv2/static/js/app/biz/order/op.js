define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.cancel = function (id, remark) {
        core.json('order/op/cancel', {id: id, remark: remark}, function (pay_json) {
    ...
    modal.delete = function (id, userdeleted) {
        core.json('order/op/delete', {id: id, userdeleted: userdeleted}, function (pay_json) {
            if (pay_json.status == 1) {
                if (modal.fromDetail) {
                    location.href = core.getUrl('order')
                } else {
    ...
    modal.finish = function (id) {
        core.json('order/op/finish', {id: id}, function (pay_json) {
            if (pay_json.status == 1) {
    ...
    modal.verify = function (orderid, verifycode) {
        ...
        core.json('verify/qrcode', {id: orderid}, function (ret) {
            if (ret.status == 0) {
