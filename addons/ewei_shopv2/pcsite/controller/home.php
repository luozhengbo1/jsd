...
class HomeController  extends Controller {
    ...
    public function index(){
        global $_W;
        $site = $this->site;
        $banner = $this->banner(1);//banner
        $casus = $this->casus(1);//案例
        $link = $this->link(1);//友情链接
        $article = $this->article(1);//文章管理
        $companyArticle = $this->companyArticle();//内容管理
    ...
    public function ajaxguestbook()
    {
        ...
        if(empty($guestbookform['nickname'])){
            die(json_encode(array('status'=>"error",'message'=>"姓名不能为空!",'type'=>'nickname')));
        }
        if(empty($guestbookform['mobile'])){
            die(json_encode(array('status'=>"error",'message'=>"电话不能为空!",'type'=>'mobile')));
        }
        if(empty($guestbookform['email'])){
            die(json_encode(array('status'=>"error",'message'=>"邮箱不能为空!",'type'=>'email')));
        }
        if(empty($guestbookform['content'])){
            die(json_encode(array('status'=>"error",'message'=>"内容不能为空!",'type'=>'content')));
        }
        ...
        if (!empty($guestbook) && $guestbook['createtime']+60 >= TIMESTAMP)
            die(json_encode(array('status'=>"error",'message'=>"距离上次留言时间小于1分钟!")));
